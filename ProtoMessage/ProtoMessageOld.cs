...
{
    public class ProtoMessageOld : IProtoMessage<ProtoMessageOld>
    {
        ...
        private readonly Dictionary<string, List<ProtoMessageOld>> _mElements =
            new Dictionary<string, List<ProtoMessageOld>>();

        private List<string> _nonParsedList;
        private List<string> _outList; // just lists for different processes with TryGetValue
        private List<ProtoMessageOld> _protoList;
        ...
        public List<ProtoMessageOld> GetElementList(string name)
        {
            if (_text.Count != 0)
            {
                ParseMessageBody(_text.ToArray());
            ...
            return new List<ProtoMessageOld>();
        }

        public ProtoMessageOld GetElement(string name)
        {
        ...
        private void ParseMessageBody(string[] lines)
        {
            ...
            var otherProto = new ProtoMessageOld(); // ProtoMessage level > 0
            ...
            foreach (string line in lines.Where(l => l != "")) // last line == "", when parse "msg" attribute
            {
                if (line.Length == 1) // where line == "}" (first level, without spaces)
                {
                    ...
                    {
                        _mElements.Add(protoName, new List<ProtoMessageOld>());
                    }

                    _mElements[protoName].Add(otherProto);
                    otherProto = new ProtoMessageOld();
                    continue;
                }
